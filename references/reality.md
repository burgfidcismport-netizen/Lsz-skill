...
确实需要个人材料时，每轮最多问三句。做了什么，哪里出乎意料，现在怎样理解。用户仍在探索选题时，可以根据回答继续下一轮，直到主旨和材料足够明确。用户要求先写，就用现有信息完成，缺口保持诚实。
...
热点选题也属于现实写作。提出方向以前，先查事件实际发生时间、报道发布时间和最新进展。旧闻重新登上热榜时，要说清它为什么再次出现。

一条热榜、一个自媒体标题或一段广泛转发的视频只能当线索。至少找到能够确认核心事实的一手来源，再用可靠媒体交叉核对背景、争议和影响。只有传言而没有可靠确认时，不把它列成可写的事实题。

把候选方向分成已经成立的事实、仍待回答的问题和作者可以形成的判断。用户看到选题时应当知道哪里可以发表看法，也知道哪些事实不能随意改写。

热度很高但没有材料、没有具体受影响的人、只能复述情绪的事件，不推荐成长文选题。正在快速变化的事件要标出信息截止时间，并优先选择即使后续事实变化也仍然成立的观察角度。
