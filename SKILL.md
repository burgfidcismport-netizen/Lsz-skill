---
name: sz-write
description: 通用中文热点选题、选题共创、材料研究、文章规划、创作与改稿 Skill。用户完全没有想法时，先检索并核验近期热点事件和新闻，提供有分歧、有材料、能展开的方向供选择；用户只有模糊思考时，通过多轮对话澄清文章主旨，再研究相关材料，与用户确认具体选题、框架和内容；用户已有明确需求时可以直接进入写作。用于知乎回答、论坛长帖、公众号文章、博客、评论、人物故事、历史叙事、新闻与行业解读、科普、教程、评测、个人叙事、小说、故事、对白、口播和演讲稿。默认写成一个见过事、查过材料、愿意把来龙去脉讲清楚的人在说话，重点保留中文互联网长回答与长帖的活人感和自然中文韵律，避免空泛的机构腔、喊口号式演说腔、营销腔和模型腔。现实内容核验事实、引语、数据与用户亲历，虚构内容可以创造人物、场景、对白、心理与情节。成稿正文严禁冒号、破折号、“不是……而是……”及同类翻案句，并清除商业黑话和模型惯用黑话。不创建作者画像、个人规则库或个人写作 Skill。
---
...
不要假定用户已经有选题、材料和完整观点。写作可以从一个念头、一种不舒服、一段观察或一句说不清的话开始。

收到需求后，先判断当前处于哪一种状态。

- 用户没有选题，也没有明确思考。先检索近期热点事件和新闻，从中筛出值得写、能够核验、适合继续思考的方向供他选择。已有明确兴趣或禁区时据此筛选，没有时保持题材分散。
- 用户有零散思考，没有具体选题。先复述自己听到的意思，区分已经确定的判断和仍然模糊的地方，再围绕分歧、经验与表达目的追问。
- 用户有选题，没有主旨。提出两到四个可以真正写开的角度，说明每个角度要回答的问题、可能得出的判断和需要什么材料，让用户选择、合并或否定。
- 用户已经给出明确选题、主旨、材料和形式。确认没有关键歧义后，直接进入材料检查，不强迫他重复走一遍选题流程。

### 没有思路时从热点中找方向

用户连零散思考也没有时，不逼他凭空回答“最近在意什么”。先用检索打开局面。

- 每次重新检索当前信息，不能凭记忆假装知道今天的热点。默认查看最近七天，优先关注最近七十二小时真正发生或出现新进展的事件。用户指定“今天”“本周”或某个领域时，按指定范围调整。
- 核对事件发生时间、报道发布时间和当前进展。平台热榜只能提供线索，不能单独证明事实或热度。优先用一手来源确认事实，再用可靠媒体了解争议和影响。
- 区分新近发生的事件、旧闻重新传播、尚未证实的传言和纯粹的话题炒作。事实仍快速变化时，标明信息截止时间，避免把暂时状态写成定论。
- 选出四到六个彼此不同的候选方向。热点本身不是入选理由。优先选择影响到具体人、有真实分歧、能找到材料，也给作者留下判断空间的事情。
- 每个方向简短交代发生了什么、为什么现在可以写、文章能追问哪一个问题、存在哪两种有依据的看法。再给一个容易让用户开口的切入问题，帮助他判断自己有没有反应。
- 不把同一条新闻改写成几个相似题目。一个事件已经被写烂时，可以向后追问它改变了谁的工作、成本或选择，也可以提炼成一个仍与事件紧密相连的长期问题。
- 不拿灾难、伤亡、隐私和未经核实的指控凑选题。对正在发展的悲剧或高风险事件，优先提供事实说明与现实影响角度，避免消费当事人。

用户选中一个方向后，先问他对哪一点有反应、倾向相信哪种解释，以及是否有相关经历。随后进入多轮澄清。用户对所有方向都无感时，根据他排除的理由更换领域或尺度，再检索一轮，不劝他勉强选。

### 多轮澄清怎么进行

探索阶段的目标是和用户一起把文章想清楚，不是尽快替他生成一个标题。

- 每轮只问一到三个会改变选题或主旨的问题。先接住用户刚提供的内容，再问下一步，不一次扔出完整问卷。
- 问具体问题。优先追问他为什么在意、亲眼见过什么、哪里拿不准、希望读者读完改变哪一个判断。少问“还有什么要求”这类空问题。
- 用户回答后，简短更新当前理解，指出新出现的选择或矛盾。不要机械重复用户原话。
- 信息足够时，提出两到四个候选选题。每个选题都要有一个核心问题、一个暂定判断和一条可展开的材料路径。题目可以稍后再润色，不用先造吸睛标题。
- 用户可以选一个、合并两个，也可以全部否定。没有得到确认，就继续澄清，不能把暂定方向当成最终主旨。
- 最终主旨至少要说清文章准备回答什么、作者倾向怎样判断、判断到哪里为止。读者、平台、篇幅和文体只确认那些会实际改变写法的部分。

用户明确说“帮我想选题”时，本轮只做选题共创，不抢先交完整文章。用户没有任何想法时，先按热点选题流程提供选择。用户要求直接写时，如果需求已经明确，可以跳过确认；需求仍然含混，就先问最关键的问题。

### 定题以后再研究和搭框架

用户确认主旨后，再寻找能支持、修正或反驳它的材料。现实题材优先查公开来源，个人经历优先追问用户，虚构题材改为补人物目标、事件和场景变化。

研究不能只为暂定观点找证据。遇到会明显改变判断的材料，要把变化告诉用户，重新确认主旨。不要悄悄换题。

材料够用以后再提出具体框架。框架应当写清每一部分准备处理什么问题、使用哪件材料、怎样推进到下一部分，以及哪些内容暂时不写。不要只给“背景、现状、问题、建议”这类空目录。
...
用户主动授权直接推进时，可以在同一轮完成研究、框架和成稿。仍要在内部完成这两个确认点对应的判断，不能因为少了一次对话就省略。

## 定题后先看作品靠什么站住

这一步先于最终框架和动笔。用户要求的字数不能跳过它。
...
进入成稿阶段后，现实材料不够时，只能选择一种处理。

- 事实型题目有公开材料可查时，先研究，研究后重新计数。可用检索工具却没有检索，仍然算没有材料。
- 个人体验或私人判断需要用户材料，每轮最多问三个问题，此时不要同时交稿。用户仍在探索选题时，按前面的多轮澄清流程继续，不把一次追问当成最后期限。
...
- 真人、历史、新闻、产品、数据、评测、教程、商业信息和用户亲历，另读 `references/reality.md`。根据热点事件和新闻帮助用户选题时也要读取。
...
现实材料不够时先停在这里。能查到公开材料就研究。作品依赖用户体验、情绪和私人判断，就在当前一轮问下面三个问题。
...
个人经历缺少关键材料时，每轮最多问三句，可以根据回答继续下一轮。用户明确要求不追问或直接出稿，就先研究，随后缩小题目或缩短篇幅，不能把新增经历安到用户身上。
